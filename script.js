  ...
  card.innerHTML = `
    ...
    <div class="p-6 cursor-pointer flex-1 flex flex-col justify-between" onclick="window.location.href='product.html?slug=${slug}'">
      <div>
        ...
        <p class="text-sm text-outline mb-4 line-clamp-2">${p.description || p.metaDescription || 'Premium component.'}</p>
      </div>
...
async function initProductPage() {
  ...
  // Creates a clean plain-text string, stripping accidental HTML tags if they exist
  const rawDesc = product.metaDescription || product.description || 'Premium component.';
  const tempDiv = document.createElement('div');
  tempDiv.innerHTML = rawDesc;
  metaDesc.content = tempDiv.textContent || tempDiv.innerText || '';
  ...
  canonicalLink.href = window.location.origin + window.location.pathname + '?slug=' + urlSlug;

  // 3. Keywords (Detected and generated directly from the title identically to canonical slug structure)
  ...
  // Replacing canonical hyphens with comma space automatically formats proper keywords based on the title context.
  metaKeywords.content = urlSlug.replace(/-/g, ', ');
  // ====================================
  ...
  if (metaDescEl) metaDescEl.textContent = product.description || product.metaDescription || 'No brief summary available for this item.';
